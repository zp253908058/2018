...
---------拣选单据begin
-- drop table pda_pickBill
IF not exists (select 1 from dbo.sysobjects where id = object_id(N'[dbo].[pda_pickBill]') and OBJECTPROPERTY(id, N'IsUserTable') = 1)
  begin
    CREATE TABLE [dbo].pda_pickBill(
      id INT IDENTITY (1,1),
      billid INT NOT NULL DEFAULT 0,--拣货单id
      billnumber VARCHAR(30) NOT NULL DEFAULT '',--拣货单单单号
      OrderNO varchar(30) not null default '', --销售订单号
      e_id INT NOT NULL DEFAULT 0,--业务员id,=pda_employees.e_id,
      pickEid int not null default 0,  --拣货人，默认=0， 触发ts_pda_getpickbill后 更新为当前登录人员的单据
      s_id INT NOT NULL DEFAULT 0,--仓库
      cname varchar(100) not null default '', --客户名称
      total numeric(18,4) default 0,  --金额
      detailcount int default 0,  --明细条数(总进度)
      boxNo varchar(200) not null default(''), --周转箱号, 存储格式为  01,02,03
      billstates INT NOT NULL DEFAULT 10, --拣货单状态 10未处理、处理中 13已完成
      pdastates INT NOT NULL DEFAULT 0,--交换状态 0提供 1pda已读取 2pda已回写
      createTime DATETIME NOT NULL DEFAULT 0,--提供时间
      pdaWrTime DATETIME NOT NULL DEFAULT 0,--回写时间,最后提交时间
    ) ON [PRIMARY]
  end
...
--拣选单据明细,pda回写PDAQTY，不能修改原始QTY
-- drop table pda_PutOnBill_D
IF not exists (select 1 from dbo.sysobjects where id = object_id(N'[dbo].[pda_pickBill_D]') and OBJECTPROPERTY(id, N'IsUserTable') = 1)
  begin
    CREATE TABLE [dbo].pda_pickBill_D(
      smb_id INT IDENTITY(1,1),--拣货单明细表id
      bill_id INT NOT NULL DEFAULT 0,--拣货单id
      p_id INT NOT NULL DEFAULT 0, --商品id,=pda_Products.p_id
      MakeDate	DATETIME NOT NULL DEFAULT 0,--生产日期
      Validdate	DATETIME NOT NULL DEFAULT 0,--效期
      Batchno	VARCHAR(20) NOT NULL DEFAULT '',--批号
      Qty	NUMERIC(18,4) NOT NULL DEFAULT 0,--原始数量
      TaxPrice	NUMERIC(18,4) NOT NULL DEFAULT 0,--含税单价
      TaxTotal	NUMERIC(18,4) NOT NULL DEFAULT 0,--含税金额
      CostPrice	NUMERIC(18,4) NOT NULL DEFAULT 0,--成本单价
      CostTotal	NUMERIC(18,4) NOT NULL DEFAULT 0,--成本金额
      ...
      wctime DATETIME NOT NULL DEFAULT 0,--完成时间
      LineSort  int NOT NULL DEFAULT 0, --原始行号，复制行时，需要保留该数值不变，用于关联ERP原始明细 0531,复制的行smb_id=0
      DealStates  int NOT NULL DEFAULT 0,--处理状态，0 未处理, 1 PDA已经处理（上架） 0531
      pdastates INT NOT NULL DEFAULT 0,--状态 0提供数据 1pda回写数据
      RecMark INT NOT NULL DEFAULT 0,--提供的数据为1,PDA新增的数据为0,方便回写数据区分原有数据和PDA新增数据
    ) ON [PRIMARY]
  end
GO
...
IF not exists (select 1 from dbo.sysobjects where id = object_id(N'[dbo].[pda_pickBill_D_Turnover]') and OBJECTPROPERTY(id, N'IsUserTable') = 1)
  begin
    CREATE TABLE [dbo].pda_pickBill_D_Turnover(
      id INT IDENTITY (1,1),
      smb_id INT NOT NULL DEFAULT 0,--拣货单据id
      turnover VARCHAR(20) NOT NULL DEFAULT ''   --周转箱
    )ON [PRIMARY]
  end
GO
...
--门店基础信息表
IF not exists (select 1 from dbo.sysobjects where id = object_id(N'[dbo].[pda_company]') and OBJECTPROPERTY(id, N'IsUserTable') = 1)
  begin
    CREATE TABLE [dbo].pda_company (
      id INT IDENTITY (1,1),
      y_id INT NOT NULL DEFAULT 0,--门店或分公司id
      serial_number VARCHAR(26) NOT NULL DEFAULT '',--编码
      pinyin VARCHAR(80) NOT NULL DEFAULT '',--拼音
      name VARCHAR(80) NOT NULL DEFAULT '',--名称
      alias VARCHAR(30) NOT NULL DEFAULT '',--简名
      ...
      RoadName VARCHAR(30) NOT NULL DEFAULT '',--路线
      ZT int not null DEFAULT 0,   --状态，默认0 ，读取后 1， 读取出错 -1  0531
      CONSTRAINT [PK_pda_company] PRIMARY KEY CLUSTERED
        (
          y_id ASC
        )
    ) ON [PRIMARY]
...
-- drop table pda_ShopRecBarcode 收货单条码表，扫描同一billid 的任意条码，自动读取出单据信息
IF not exists (select 1 from dbo.sysobjects where id = object_id(N'[dbo].[pda_ShopRecBarcode]') and OBJECTPROPERTY(id, N'IsUserTable') = 1)
  begin
    CREATE TABLE [dbo].pda_ShopRecBarcode(
      smb_id INT IDENTITY(1,1),--收货单条码表id
      bill_id INT NOT NULL DEFAULT 0,--收货单id, =pda_CheckBill.billid
      PickType INT NOT NULL DEFAULT 0,--1整件 2拼箱 3打包
      barcode VARCHAR(50) NOT NULL DEFAULT '',--条码
    ) ON [PRIMARY]
  end
...
--
IF not exists (select 1 from dbo.sysobjects where id = object_id(N'[dbo].[pda_ShopRecBill]') and OBJECTPROPERTY(id, N'IsUserTable') = 1)
  begin
    CREATE TABLE [dbo].pda_ShopRecBill(
      id INT IDENTITY (1,1),
      billid INT NOT NULL DEFAULT 0,--收货单id
      billnumber VARCHAR(30) NOT NULL DEFAULT '',--门店收货单号
      y_id INT NOT NULL DEFAULT 0,--往来机构(门店)id,=pda_clients.c_id
      e_id INT NOT NULL DEFAULT 0,--发货单业务人id,=pda_employees.e_id
      rec_eid int not null default 0,  --收货人id， pda 处理后回写
      billstates INT NOT NULL DEFAULT 10,--状态 10未处理 13已收货
      pdastates INT NOT NULL DEFAULT 0,--交换状态 0提供 1pda已读取 2pda已回写
      pdaInTime DATETIME NOT NULL DEFAULT 0,--提供时间
      pdaReTime DATETIME NOT NULL DEFAULT 0,--读取时间
      pdaWrTime DATETIME NOT NULL DEFAULT 0,--回写时间
...
IF not exists (select 1 from dbo.sysobjects where id = object_id(N'[dbo].[pda_ShopRecBill_D]') and OBJECTPROPERTY(id, N'IsUserTable') = 1)
  begin
    CREATE TABLE [dbo].pda_RecShopBill_D(
      smb_id INT IDENTITY(1,1),--收货单明细表id
      bill_id INT NOT NULL DEFAULT 0,--收货单id,=pda_ShopRecBill.billid
      p_id INT NOT NULL DEFAULT 0,--商品id,=pda_Products.p_id
      MakeDate	DATETIME NOT NULL DEFAULT 0,--生产日期
      Validdate	DATETIME NOT NULL DEFAULT 0,--效期
      Batchno	VARCHAR(20) NOT NULL DEFAULT '',--批号
      Yqty	NUMERIC(18,4) NOT NULL DEFAULT 0,--原始数量（配送数量）
      EligibleQty	NUMERIC(18,4) NOT NULL DEFAULT 0,--收货数量
      TaxPrice	NUMERIC(18,4) NOT NULL DEFAULT 0,--含税单价
      TaxTotal	NUMERIC(18,4) NOT NULL DEFAULT 0,--含税金额
      S_id	INT NOT NULL DEFAULT 0,--仓库id,=pda_storages.s_id
      ...
      original_id INT NOT NULL DEFAULT 0,
      loginid INT NOT NULL DEFAULT 0,
      LineSort  int NOT NULL DEFAULT 0, --原始行号，复制行时，需要保留该数值不变，用于关联ERP原始明细
      wctime DATETIME NOT NULL DEFAULT 0, --完成时间
      DealStates  int NOT NULL DEFAULT 0,--处理状态，0 未处理, 1 已经处理
      pdastates INT NOT NULL DEFAULT 0,--状态 0提供数据 1pda回写数据
    ) ON [PRIMARY]
...
--拣货 存储过程
/****** Object:  StoredProcedure [dbo].[ts_pda_getPickbill]    Script Date: 03/15/2018 19:47:59 ******/
IF  EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[dbo].[ts_pda_getPickbill]') AND type in (N'P', N'PC'))
  DROP PROCEDURE [dbo].[ts_pda_getPickbill]
GO
...
CREATE PROCEDURE [dbo].[ts_pda_getPickbill]
  (
    @e_id INT
  )
AS
  ...
  DECLARE @tmpGsp TABLE(id int) --本次下传的单据id
  DECLARE @tmpKq TABLE(sa_id INT) --该职员可以拣货的库区id

  DECLARE @billid INT --需要返回的单据编号

  SET @billid = 0

  INSERT INTO @tmpGsp( id )
    SELECT TOP 5 Gspbillid FROM dbo.GSPbillidx
    WHERE BillType = 541 AND BillStates = 15 AND PdaFlag = 0 AND WMS_IsPDA = 1--只传pda 拣货的单据

  INSERT INTO dbo.pda_pickBill
  ( billid ,billnumber ,OrderNO ,e_id ,pickEid ,s_id ,cname ,total ,
    detailcount ,boxNo ,billstates ,pdastates ,createTime ,pdaWrTime,WmsLevel,Sa_id
  )

    SELECT g.Gspbillid ,billnumber ,g.Ybillid ,g.AuditMan1 ,0 ,s_id ,c.name ,g.TaxTotal,
      g.WMS_detailcount ,g.WMS_BoxNo ,billstates ,0 ,GETDATE() ,0,WmsLevel,g.WMS_sa_id
    FROM dbo.GSPbillidx g,@tmpGsp t,dbo.clients c
    WHERE g.Gspbillid = t.id AND g.C_id = c.client_id

  INSERT INTO dbo.pda_pickBill_D
  ( bill_id ,p_id ,MakeDate ,Validdate ,Batchno ,Qty ,pickQty ,TaxPrice ,
    TaxTotal ,S_id ,Sa_id ,Location_id ,wctime ,LineSort ,DealStates
  )
    SELECT gd.Gspbill_id ,p_id ,MakeDate ,Validdate ,Batchno ,gd.Yqty ,pickQty ,TaxPrice ,
      TaxTotal ,gd.S_id ,l.sa_id ,Location_id ,0 ,gd.Gspsmb_id ,0
    FROM dbo.GSPbilldetail gd,@tmpGsp t,dbo.location l
    WHERE gd.Gspsmb_id = t.id AND gd.Location_id = l.loc_id

  --更新状态
  UPDATE GSPbillidx SET PdaFlag = 1 FROM @tmpGsp t WHERE Gspbillid = t.id

  --需要根据角色判断是否该职员是否可以拣货
  INSERT INTO @tmpKq( sa_id )
    SELECT distinct r.Store_KQ_ID FROM limgroup g,wms_limgroupuser u,WMSEmpRoleAQ AQ ,dbo.WMSRegion r
    WHERE g.gid = u.lgid AND u.lid = AQ.Bill_id AND AQ.RoleAQid = r.ID AND
          u.e_id = @e_id AND g.name ='拣货员'

  SELECT TOP 1 @billid = billid FROM pda_pickBill pb,@tmpKq kq
  WHERE pb.pdastates = 0 AND pb.Sa_id = kq.sa_id
  ORDER BY WmsLevel

  UPDATE dbo.pda_pickBill SET e_id = @e_id

  SELECT * FROM dbo.pda_pickBill WHERE billid = @billid
